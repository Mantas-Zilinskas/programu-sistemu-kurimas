...
namespace EmocineSveikataServer.Services.SmsService
{
    public class TwilioSmsService : ISmsService
    {
        ...
        public TwilioSmsService(ILogger<TwilioSmsService> logger, IOptions<TwilioSettings> twilioSettings)
        {
            _logger = logger;
        ...
        public async Task<bool> SendDailyWellnessMessageAsync(string phoneNumber, string[]? topics = null)
        {
            try
            {
                ...
                _logger.LogInformation($"Daily wellness message sent to {phoneNumber}. Status: {result.Status}. SID: {result.Sid}");
                
                return result.Status != MessageResource.StatusEnum.Failed;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error sending wellness message to {phoneNumber}: {ex.Message}");
                return false;
            }
        }
